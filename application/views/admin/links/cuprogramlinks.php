  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Program To Course Link</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="<? echo base_url("admin/dashboard") ?>">Home</a></li>
              <li class="breadcrumb-item"><a href="<? echo base_url("admin/links/programLinks") ?>">Program To Course Links</a></li>
        	...
        	<div class="col-md-12">
				<div class="card card-primary card-outline">
					<div class="card-header">
						<h3 class="card-title"><? echo ($id != "") ? 'Update' : 'Create'; ?> Program To Course Link</h3>
					</div>
					<div class="card-body">
							...
							<div class="row">
								...
								<div class="col-md-12">
									<div class="form-group">
									  <label>Courses</label>
									  <select class="duallistbox" multiple="multiple" name="course[]" style="height: 200px !important;">
										<? 
										  	$subjects = $this->db->order_by("id","desc")->get_where("tbl_courses",["deleted"=>0])->result(); 
										  	foreach($subjects as $subject){
												...
												echo '<option value="'.$subject->id.'" '.$sel.'>'.$subject->course_name.'</option>';
											}
										 ?>
									  </select>
									</div>
								...
								<div class="col-md-6">
									<div class="form-group">
									  <label>Program</label>
									  <select class="form-control select2bs4" name="program" required style="width: 100%;">
										<option value="">Select Program</option>
										<? 
										  	$scats = $this->db->order_by("id","desc")->get_where("tbl_programs",["deleted"=>0])->result(); 
										  	foreach($scats as $scat){
												$sel = ($id == $scat->id) ? 'selected' : '';
												echo '<option value="'.$scat->id.'" '.$sel.'>'.$scat->program_name.'</option>';
											}
										 ?>
									  </select>
									</div>
...
<script>
	...
	$("#slcategory").submit(function(e){
		...
		$.ajax({
			...
			data : fdata,
			dataType: "json",
			url : "<? echo base_url('apis/Links/course_program_link') ?>",
